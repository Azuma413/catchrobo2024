...
pid_param gm6020_location(0.2,    0.1,    0.02,   5,      350);
...
CybergearSoftwareConfig config1(cyber_ids[0], CCW, 30, 27, 12, 4.0*M_PI, -4.0*M_PI, CW, 0);
CybergearSoftwareConfig config2(cyber_ids[1], CCW, 30, 27, 12, 4.0*M_PI, -4.0*M_PI, CCW, 0);
...
bool calib_motors(){
    ...
    while(true){ // 速度が一定以下になったら停止
        ...
        if(std::abs(motor_status[1].velocity) < calib_speed && !flag2){
            ...
            delay(100);
            controller.send_position_command(cyber_ids[1], 30*M_PI/90);
...
void loop() {
    ...
    if (mode == 0) { // 接続テスト
        ...
        // id1 cyber1 第二関節
        target = (target_angle[0])*M_PI/45; // 1/4倍減速
        controller.send_position_command(cyber_ids[0], target);
        // id2 cyber2 第一関節
        target = (target_angle[1] + 30)*M_PI/90; // 1/2倍減速
        controller.send_position_command(cyber_ids[1], target);
        // id3 gm1 id3 腕
        target = target_angle[2] + 150;
        if (target < 0) target += 360;
        gm6020_1.set_angle(target);
        // id4 gm2 id4 手首
        target = target_angle[3] + 360;
